...
class ResultPage(BasePage):
    ...
    def correct_rate(self):
        """准确率"""
        rate = self.driver\
            .find_element_by_id("com.vanthink.student.debug:id/correct_rate").text
        return rate
    ...
    def result_score(self):
        """积分"""
        score = self.driver \
            .find_element_by_id("com.vanthink.student.debug:id/score").text
        return score
    ...
    def result_star(self):
        """星星"""
        star = self.driver \
            .find_element_by_id("com.vanthink.student.debug:id/star").text
        return star

    @teststep
    def result_time(self):
        """时间"""
        ele = re.sub("\D", "", self.driver.find_element_by_id("com.vanthink.student.debug:id/time").text)
        return ele
    ...
    def rank(self):
        """title: 排行榜"""
        rank = self.driver\
            .find_element_by_id("com.vanthink.student.debug:id/rank").text
        return rank
    ...
    def rank_index(self):
        """排名"""
        item = self.driver \
            .find_elements_by_id("com.vanthink.student.debug:id/index")
        return item
    ...
    def rank_name(self):
        ...
        time.sleep(1)
        item = self.driver \
            .find_elements_by_id("com.vanthink.student.debug:id/name")
        return item
    ...
    def rank_accuracy_rate(self):
        """准确率最高的那次的正确率"""
        item = self.driver \
            .find_elements_by_id("com.vanthink.student.debug:id/rate")
        return item
    ...
    def rank_spend_time(self):
        """准确率最高的那次 完成所用时间"""
        item = self.driver \
            .find_elements_by_id("com.vanthink.student.debug:id/time")
        return item
    ...
    def mine_result(self):
        """查看答案 页面每个小题后面 对错标识"""
        item = self.driver \
            .find_element_by_id("com.vanthink.student.debug:id/iv_mine").get_attribute('selected')
        return item
    ...
    def result_page_correct_rate(self, count, rate):
        ...
        if self.wait_check_result_page():  # 结果页检查点
            if count != 0:  # 答对的题数
                correct_rate = re.sub("\D", "", self.correct_rate())  # 本次结果统计——准确率
    ...
    @teststeps
    def result_all_infor(self):
        correct_rate = re.sub("\D", "", self.correct_rate())
        score = re.sub("\D", "", self.result_score())  # 本次结果统计——积分
        star_count = re.sub("\D", "", self.result_star())  # 本次结果统计——星星
        now_time = re.sub("\D", "", self.result_time())  # 本次结果统计——所用时间
        print('准确率:%s, 积分:%s， 星星:%s，所用时间:%s' % (correct_rate, score, star_count, now_time))
