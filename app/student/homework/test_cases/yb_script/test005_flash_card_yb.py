...
class Games(unittest.TestCase):
    ...
    def test_flash_card_yb(self):
        ...
        if self.home_page.wait_check_page():  # 页面检查点
            ...
            if gv.FLA_CARD_YB in var[0]:  # 该作业存在
                ...
                print('当前页no have该作业')
                game = self.home_page.swipe(var[0], gv.FLA_CARD_YB, '闪卡练习')  # 作业list翻页
    ...
    def flash_card_study(self, index):
        ...
        time.sleep(3)
        # rate = self.flash_card.rate()  # 获取总题数
        self.study_pattern()  # 闪卡练习 学习游戏过程
        time.sleep(2)

        self.homework.back_up_button()  # 结果页 返回按钮
        # if self.flash_card.study_sum():  # 结果页检查点
        #     self.flash_card.result_page(rate, answer)  # 点击结果页听力按钮 和 star按钮
    ...
    def flash_card_copy(self, index):
        ...
        time.sleep(2)
        # rate = self.flash_card.rate()  # 获取总题数
        self.copy_pattern()  # 闪卡练习 抄写模式 游戏过程
        self.homework.back_up_button()  # 结果页 返回按钮
        time.sleep(2)
        # if self.flash_card.study_sum():  # 结果页检查点
        #     self.flash_card.result_page(rate, answer)  # 点击结果页听力按钮 和 star按钮
    ...
    def study_pattern(self):
        ...
        if self.flash_card.wait_check_page():   # 页面检查点
            ...
            for i in range(int(rate)):
                word = self.flash_card.english_study()
                explain = self.flash_card.explain_study()
