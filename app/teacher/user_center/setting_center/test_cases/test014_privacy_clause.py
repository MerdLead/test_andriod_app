...
from app.teacher.user_center.user_information.object_page.user_center_page import TuserCenterPage, PrivacyPage
        ...
        if self.home.wait_check_page():  # 页面检查点
            ...
            if self.user.wait_check_page():  # 页面检查点
                ...
                if self.setting.wait_check_page():
                    self.setting.privacy_clause()  # 进入隐私条款页面

                    if self.privacy_clause.wait_check_page():  # 页面检查点
                        for i in range(4):
                            print('翻页%s次' % (i+1))
